...
//        var_dump($banner);
        ...
        $info1= Yii::$app->db->createCommand("select id,content from {{%info}} where cate='公开课' order by id desc limit 5")->queryAll();
        $info2= Yii::$app->db->createCommand("select id,title from {{%info}} where cate='公告'order by id desc limit 5")->queryAll();
        $info2= Yii::$app->db->createCommand("select id,title,summary from {{%info}} order by id desc limit 5")->queryAll();

//        var_dump($info);die;
        return $this->renderPartial('index',['data'=>$data,'banner'=>$banner,'teachers'=>$teachers,'info1'=>$info1,'info2'=>$info2]);
    }
}