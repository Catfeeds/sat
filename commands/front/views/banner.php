<nav class="nav-wrap">
    <div class="w10" style="overflow: inherit;">
        <a href="/"><img class="logo" src="/cn/images/sy-logo.png" alt=""></a>
        <ul class="inb nav-list clearfix">
            <li><a class="on on-hover nav1-link" href="/">SAT培训</a></li>
            <li><a class="nav1-link" href="/gmatsheet.html">GMAT培训</a></li>
            <li><a class="nav1-link" href="/toelflsheet.html">托福培训</a></li>
            <li><a class="nav1-link" href="/ieltssheet.html">雅思培训</a></li>
        </ul>
    </div>
</nav>